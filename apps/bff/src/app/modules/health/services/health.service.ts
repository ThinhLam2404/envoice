...
import { Transport, RedisOptions, TcpClientOptions } from '@nestjs/microservices';
...
export class HealthService {
  ...
  checkReadiness() {
    const tcpServices = [
      {
        key: TCP_SERVICES.INVOICE_SERVICE,
        configKey: 'TCP_SERV.TCP_INVOICE_SERVICE',
      },
      {
        key: TCP_SERVICES.PRODUCT_SERVICE,
        configKey: 'TCP_SERV.TCP_PRODUCT_SERVICE',
      },
      {
        key: TCP_SERVICES.USER_ACCESS_SERVICE,
        configKey: 'TCP_SERV.TCP_USER_ACCESS_SERVICE',
      },
      {
        key: TCP_SERVICES.AUTHORIZER_SERVICE,
        configKey: 'TCP_SERV.TCP_AUTHORIZER_SERVICE',
      },
      {
        key: TCP_SERVICES.PDF_GENERATOR_SERVICE,
        configKey: 'TCP_SERV.TCP_PDF_GENERATOR_SERVICE',
      },
      {
        key: TCP_SERVICES.MEDIA_SERVICE,
        configKey: 'TCP_SERV.TCP_MEDIA_SERVICE',
      },
    ];
    ...
    const grpcServices = [
      {
        ...
        configKey: 'GRPC_SERV.GRPC_AUTHORIZER_SERVICE',
      },
      {
        key: GRPC_SERVICES.USER_ACCESS_SERVICE,
        configKey: 'GRPC_SERV.GRPC_USER_ACCESS_SERVICE',
      },
    ];
  ...
  private checkTcpService(key: string, configKey: string) {
    Logger.debug(
      this.configService.get(`${configKey}.options.host`),
      this.configService.get(`${configKey}.options.port`),
    );
